...
1. Сначала допишите подсчет с базовой предобработкой (токенизация, очистка, фильтр по стоп-словам) без записи результатов в файл, пока выводите в консоль. (1 балл)
2. Реализуйте запись результатов в файл по переданному пути. (1 балл)
3. Реализуйте режим предобработки *стемминг* средствами одной из популярных библиотек (`nltk`, `spaCy`). (1 балл)
4. Реализуйте режим предобработки *лемматизация* средствами одной из популярных библиотек (`nltk`, `spaCy`). (1 балл)
5. Реализуйте фильтр по частям речи, чтобы пользователь мог задать перечень релевантных частей речи (`POS`), и подсчет вёлся только по ним. Аналогично предыдущим пунктам, используйте библиотеки (1 балл).
